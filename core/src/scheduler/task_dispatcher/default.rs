...
impl From<DefaultTaskDispatcherConfig> for Arc<DefaultTaskDispatcher> {
    fn from(value: DefaultTaskDispatcherConfig) -> Arc<DefaultTaskDispatcher> {
        Arc::new(DefaultTaskDispatcher {
            critical_pool: Arc::new(tokio::runtime::Builder::new_multi_thread()
                .worker_threads(value.workload.critical)
                .thread_name("critical-task-worker")
                .build()
                .unwrap()),
            important_pool: Arc::new(tokio::runtime::Builder::new_multi_thread()
                .worker_threads(value.workload.important)
                .thread_name("important-task-worker")
                .build()
                .unwrap()),
            high_pool: Arc::new(tokio::runtime::Builder::new_multi_thread()
                .worker_threads(value.workload.high)
                .thread_name("high-task-worker")
                .build()
                .unwrap()),
            moderate_pool: Arc::new(tokio::runtime::Builder::new_multi_thread()
                .worker_threads(value.workload.moderate)
                .thread_name("moderate-task-worker")
                .build()
                .unwrap()),
            low_pool: Arc::new(tokio::runtime::Builder::new_multi_thread()
    ...
    async fn dispatch(
        ...
        let target_pool = match task.priority {
            TaskPriority::CRITICAL => {
                self.critical_pool.clone()
            }

            TaskPriority::IMPORTANT => {
                self.important_pool.clone()
            }

            TaskPriority::HIGH => {
                self.high_pool.clone()
            }

            TaskPriority::MODERATE => {
                self.moderate_pool.clone()
            }
        ...
        target_pool.spawn(async move {
            task.clone().overlap_policy.handle(task.clone(), emitter).await;
